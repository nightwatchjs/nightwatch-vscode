describe('sample google search', function () {
  ...
  it('demo test using expect apis', async function (browser) {
    ...
    const consentPresent = await browser.isPresent(
      '[aria-modal="true"][aria-label="Before you continue to Google Search"]',
    );
    ...
    await browser
      ...
      .sendKeys('form[action="/search"] input[type=text]', [
        'Nightwatch.js',
        browser.Keys.ENTER,
      ])
